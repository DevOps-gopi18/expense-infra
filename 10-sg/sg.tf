...
module "bastion_sg" {
    ...
    environment = var.environment
    sg_name = "bastion"
    sg_descrption = "Created for private instances in expense dev"
    common_tags = var.common_tags
    vpc_id = data.aws_ssm_parameter.vpc_id.value
}
...
module "vpn_sg" {
    ...
    environment = var.environment
    sg_name = "vpn"
    sg_descrption = "Created for VPN instances in expense dev"
    common_tags = var.common_tags
    vpc_id = data.aws_ssm_parameter.vpc_id.value
}
...
module "alb_ingress_sg" {
    ...
    environment = var.environment
    sg_name = "alb-ingress"
    sg_descrption = "Created for backend ALB in expense dev"
    common_tags = var.common_tags
    vpc_id = data.aws_ssm_parameter.vpc_id.value
}
